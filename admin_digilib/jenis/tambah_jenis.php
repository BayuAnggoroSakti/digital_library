...
$nipmax=mysql_fetch_array(mysql_query("SELECT max(id_ebookjenis) FROM digilib_ebookjenis"));
...
$p_id_ebookjenis = isset($_POST['id_ebookjenis']) ? $_POST['id_ebookjenis'] : "";	
...
$q_data_edit	= mysql_query("SELECT * FROM digilib_ebookjenis WHERE id_ebookjenis= '$id_daftar'");
$a_data_edit	= mysql_fetch_array($q_data_edit);
$id_ebookjenis		= $a_data_edit[0];
...
if ($p_tombol == "DAFTAR") {
	if ($p_nama == "" 
		...
		echo "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'><i class='icon-remove'></i></button><strong><i class='ace-icon fa fa-times'></i> MAAF! </strong> Form Isian Masih Belum lengkap mohon di lengkapi<br/></div>";
			...
			$q_cek_ganda = mysql_query("SELECT * FROM digilib_ebookjenis WHERE nama = '$p_nama'");
			$j_cek_ganda = mysql_fetch_array($q_cek_ganda);
			
			if ($j_cek_ganda > 0) {
				echo "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'><i class='icon-remove'></i></button><strong><i class='ace-icon fa fa-times'></i> MAAF! </strong> Jenis sudah terdaftar<br/></div>";
			} else {   
					$q_daftar	= mysql_query("INSERT INTO digilib_ebookjenis VALUES ('','$p_nama','$p_deskripsi','0')");	
					if ($q_daftar) {
					echo "<div class='alert alert-block alert-success'><button type='button' class='close' data-dismiss='alert'><i class=''></i></button><strong><i class='ace-icon fa fa-check'></i> BERHASIL</strong> Data Jenis Ebook Berhasil di simpan<br/></div></div><script>window.location ='media.php?id=list_jenis'</script>";
				} else {
	...
	if ($p_nama == "fiksi") 
		...
		$q_edit	= mysql_query("UPDATE digilib_ebookjenis SET nama='$p_nama', deskripsi='$p_deskripsi'
									where id_ebookjenis='$id_daftar'");
				
									
		if ($q_edit) {
		
		echo "<div class='alert alert-block alert-success'><button type='button' class='close' data-dismiss='alert'><i class=''></i></button><strong><i class='ace-icon fa fa-check'></i> BERHASIL</strong> Data Jenis Ebook Berhasil di Ubah<br/></div><script>window.location ='media.php?id=list_jenis'</script>";
...
<div class="widget-header widget-header-blue widget-header-flat">
	...
	<div class="widget-toolbar">
					...
					<div class="row-fluid">
						...
						<form class="form-horizontal" action="?id=tambah_jenis" method="post"  role="form">
